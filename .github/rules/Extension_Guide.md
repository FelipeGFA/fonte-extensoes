...
### 5.11 URL Activities and Deep Links

- Avoid hardcoded host checks in deep-link or URL search handling. Compare against the source's current `baseUrl` so mirrors and configurable domains keep working.
- In `UrlActivity`, catch `Throwable` instead of `Exception` to avoid unhandled activity crashes. Keep business logic in the source class, not the activity.
