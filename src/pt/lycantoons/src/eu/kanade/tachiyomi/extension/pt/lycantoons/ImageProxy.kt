...
class ImageProxy(
    ...
    private fun downloadImageWithWebView(url: String): WebViewImage? {
        ...
        val session = ImageDownloadSession(
            file = File.createTempFile("lycan-image-", ".tmp", context.cacheDir),
    ...
    private fun enqueueImageFetch(id: Int, url: String) {
        val script = "window.lycanFetchImage($id, ${url.toJsStringLiteral()});"
    ...
    private companion object {
        const val IMAGE_BRIDGE_INTERFACE = "LycanImageBridge"
        ...
        val IMAGE_FETCH_HTML = """
            <!doctype html>
            <script>
            window.lycanFetchImage = function(id, url) {
              function passChunk(value) {
                ...
                for (var i = 0; i < value.length; i += chunkSize) {
                  ...
                  window.LycanImageBridge.passChunk(id, btoa(binary));
                }
              }
              ...
              fetch(url, { credentials: "include" })
                .then(function(response) {
                  if (!response.ok || !response.body || !response.body.getReader) {
                    window.LycanImageBridge.passError(id);
                    return;
                  }

                  window.LycanImageBridge.startImage(id, response.headers.get("content-type") || "");
                  ...
                  function read() {
                    reader.read()
                      .then(function(result) {
                        if (result.done) {
                          window.LycanImageBridge.finishImage(id);
                          return;
                        }
                      ...
                      .catch(function() { window.LycanImageBridge.passError(id); });
                  }
                  read();
                })
                .catch(function() { window.LycanImageBridge.passError(id); });
