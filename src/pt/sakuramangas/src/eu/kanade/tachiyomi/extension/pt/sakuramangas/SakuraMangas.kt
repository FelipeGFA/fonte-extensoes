...
class SakuraMangas : HttpSource() {
    ...
    @Volatile private var cachedSecurityKeys: SecurityKeys? = null

    @Volatile private var securityKeysTimestamp: Long = 0
    private val securityKeysTtl = 30 * 60 * 1000L

    @Volatile private var cachedImageHeaders: Pair<String, String>? = null

    @Volatile private var imageHeadersTimestamp: Long = 0
    ...
    private fun securityHeaders(token: String, securityKeys: SecurityKeys?, referer: String, cookies: String) = ajaxHeaders(referer).newBuilder()
    ...
    private fun getCachedScript(url: String): String? {
        ...
        return if (System.currentTimeMillis() - cached.timestamp < scriptCacheTtl) cached.content else null.also { scriptCache.remove(url) }
    }
    ...
    private fun getSecurityKeys(pageUrl: String, cookies: String): SecurityKeys? {
        ...
        if (cachedSecurityKeys != null && (now - securityKeysTimestamp) < securityKeysTtl) return cachedSecurityKeys

        return try {
            val document = client.newCall(
                Request.Builder().url(pageUrl).headers(headers.newBuilder().set("Cookie", cookies).build()).get().build(),
            ).execute().use { if (!it.isSuccessful) return null; it.asJsoup() }

            val securityScriptUrl = document.selectFirst("script[src*=manga], script[src*=capitulo]")?.absUrl("src") ?: return null
            val normalizeScriptUrl = document.selectFirst("script[src*=normalize]")?.absUrl("src")

            val securityScript = getCachedScript(securityScriptUrl)
                ?: client.newCall(GET(securityScriptUrl, scriptHeaders())).execute().use { if (it.isSuccessful) it.body.string().also { c -> scriptCache[securityScriptUrl] = CachedScript(c, now) } else "" }

            val normalizeScript = normalizeScriptUrl?.let {
                getCachedScript(it) ?: client.newCall(GET(it, scriptHeaders())).execute().use { r -> if (r.isSuccessful) r.body.string().also { c -> scriptCache[it] = CachedScript(c, now) } else "" }
            }.orEmpty()
    ...
    private fun getImageHeaders(chapterUrl: String, cookies: String): Pair<String, String>? {
        val now = System.currentTimeMillis()
        if (cachedImageHeaders != null && (now - imageHeadersTimestamp) < imageHeadersTtl) return cachedImageHeaders

        return try {
            val document = client.newCall(
                Request.Builder().url(chapterUrl).headers(headers.newBuilder().set("Cookie", cookies).build()).get().build(),
            ).execute().use { if (!it.isSuccessful) return null; it.asJsoup() }

            val chapterScriptUrl = document.selectFirst("script[src*=capitulo][src*=core.js]")?.absUrl("src") ?: return null

            val chapterScript = getCachedScript(chapterScriptUrl)
                ?: client.newCall(GET(chapterScriptUrl, scriptHeaders())).execute().use { if (it.isSuccessful) it.body.string().also { c -> scriptCache[chapterScriptUrl] = CachedScript(c, now) } else "" }
    ...
    override fun mangaDetailsParse(response: Response): SManga {
        ...
        response.close()
        val securityData = webViewInterceptor.captureSecurityData(url) ?: throw CloudflareRequiredException()
        val mangaInfoJson = securityData.mangaInfoJson ?: throw CloudflareRequiredException()
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        val securityData = webViewInterceptor.captureSecurityData(url) ?: throw CloudflareRequiredException()
        if (securityData.proof.isEmpty() || securityData.challenge.isEmpty() || securityData.mangaId.isEmpty() || securityData.csrfToken.isEmpty()) {
    ...
    override fun pageListParse(response: Response): List<Page> {
        val url = response.request.url.toString()
        val chapterData = webViewInterceptor.getChapterData(url) ?: throw CloudflareRequiredException()

        if (cachedImageHeaders == null || (System.currentTimeMillis() - imageHeadersTimestamp) >= imageHeadersTtl) {
            val cookies = client.cookieJar.loadForRequest(url.toHttpUrl()).joinToString("; ") { "${it.name}=${it.value}" }
    ...
    private fun fetchFilters() {
        ...
        runCatching {
            ...
            document.select("#group-demografia .btn-filter-chip").mapNotNull { el ->
                val value = el.attr("data-value"); val text = el.text()
